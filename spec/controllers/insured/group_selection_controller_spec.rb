...
RSpec.describe Insured::GroupSelectionController, :type => :controller do
  ...
  describe "test private method" do
    it "should be nil if no user exist" do
      expect(controller.send(:find_person, "test")).to  eq(nil)
    end
    it "should return user if user exist" do
      p = Person.first
      expect(controller.send(:find_person, p.id)).to eq(p)
    end
  end
