...
RSpec.describe Employers::CensusEmployeesController do
  ...
  let(:census_employee) { FactoryGirl.create(:census_employee, employer_profile_id: employer_profile.id, employment_terminated_on: TimeKeeper::date_of_record - 30.days,  hired_on: "2014-11-11") }
  ...
  describe "PUT update" do
    ...
    context "get flash notice" do
      ...
      it "with no benefit_group_id" do
        ...
        allow(controller).to receive(:benefit_group_id).and_return(nil)
        post :update, :id => census_employee.id, :employer_profile_id => employer_profile_id, census_employee: {}
        expect(flash[:notice]).to eq "Note: new employee cannot enroll on DC Health Link until they are assigned a benefit group. Census Employee is successfully updated."
      end
    end
  ...
  describe "GET terminate" do
    ...
    context "with termination date" do
      it "should terminate census employee" do
        xhr :get, :terminate, :census_employee_id => census_employee.id, :employer_profile_id => employer_profile_id, termination_date: TimeKeeper.date_of_record.to_s, :format => :js
        expect(response).to have_http_status(:success)
