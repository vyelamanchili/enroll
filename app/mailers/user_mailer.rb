class UserMailer < ApplicationMailer
  ...
  def plan_shopping_completed(user, hbx_enrollment, email)
    mail({to: email, subject: "Your #{Settings.site.short_name} Enrollment Confirmation"}) do |format|
  ...
  def agent_invitation_email(email, person_name, invitation)
    mail({to: email, subject: "#{Settings.site.short_name} Support Invitation "}) do |format|
  ...
  def generic_consumer_welcome(first_name, hbx_id, email)
    message = mail({to: email, subject: "#{Settings.site.short_name}", from: 'no-reply@individual.dchealthlink.com'}) do |format|
