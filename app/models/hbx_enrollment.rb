...
class HbxEnrollment
  ...
  def generate_hbx_signature
    enrollment_signature =  Digest::MD5.hexdigest(applicant_ids.sort.map(&:to_s).join)
  end
  ...
  def cancel_previous(year)
    ...
    self.household.hbx_enrollments.ne(id: id).by_coverage_kind(self.coverage_kind).by_year(year).cancel_eligible.by_kind(self.kind).each do |p|
      ...
      #if (p.applicant_ids.sort == self.applicant_ids.sort && p.plan.carrier_profile_id == self.plan.carrier_profile_id && p.kind != "employer_sponsored" && self.effective_on == p.effective_on) || p.kind == "employer_sponsored"
