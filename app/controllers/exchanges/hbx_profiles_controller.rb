class Exchanges::HbxProfilesController < ApplicationController
  ...
  def verification_index
    if params[:q]
      total_families = Person.search(params[:q]).map(&:families).flatten.uniq
      @families = Kaminari.paginate_array(total_families).page(params[:page]).per(3)
    else
      @families = Family.by_enrollment_individual_market.where(:'households.hbx_enrollments.aasm_state' => "enrolled_contingent").order_by(:created_at => 'asc').page(params[:page]).per(20)
    end
    respond_to do |format|
